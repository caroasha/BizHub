...
export function Header() {
  ...
  const handleProfile = () => { setDropdownOpen(false); navigate('/apartment/settings'); };
  const handlePassword = () => { setDropdownOpen(false); navigate('/apartment/settings?tab=password'); };
  const handleSwitch = (mod) => { setDropdownOpen(false); navigate(moduleRoutes[mod] || '/apartment'); };

  const adminName = user?.name || 'Admin';
  const userModules = user?.modules || [];
  const otherModules = userModules.filter(m => m !== 'apartment');
