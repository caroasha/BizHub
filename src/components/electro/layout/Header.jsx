...
export function Header() {
  ...
  const handleProfile = () => { setDropdownOpen(false); navigate('/electro/settings'); };
  const handlePassword = () => { setDropdownOpen(false); navigate('/electro/settings?tab=password'); };
  const handleSwitch = (mod) => { setDropdownOpen(false); navigate(moduleRoutes[mod] || '/electro'); };

  const adminName = user?.name || 'Admin';
  const userModules = user?.modules || [];
  const otherModules = userModules.filter(m => m !== 'electronics');
