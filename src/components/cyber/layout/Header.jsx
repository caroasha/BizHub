...
export function Header() {
  ...
  const handleProfile = () => { setDropdownOpen(false); navigate('/cyber/settings'); };
  const handlePassword = () => { setDropdownOpen(false); navigate('/cyber/settings?tab=password'); };
  const handleSwitch = (mod) => { setDropdownOpen(false); navigate(moduleRoutes[mod] || '/cyber'); };

  const adminName = user?.name || 'Admin';
  const userModules = user?.modules || [];
  const otherModules = userModules.filter(m => m !== 'cyber');
