...
export default function Sales() {
  ...
  const fetchSales = (source, status) => {
    setLoading(true);
    const params = { page, limit: 20 };
    if (source) params.source = source;
    if (status) params.paymentStatus = status;
    getSales(params).then(res => { setSales(res?.data || []); setTotalPages(res?.pagination?.totalPages || 1); }).catch(() => {}).finally(() => setLoading(false));
  };

  useEffect(() => {
    if (activeTab === 'history') fetchSales(null, 'paid,cancelled');
    if (activeTab === 'invoices') fetchSales('invoice', filterStatus || null);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const res = await createSale({ items, customerName: customerName || 'Walk-in Customer', customerPhone, customerEmail, paymentMethod, discount: Number(discount) });
  ...
  const openEditInvoice = (sale) => {
    setEditSale(sale);
    setInvoiceForm({ customerName: sale.customerName || '', customerPhone: sale.customerPhone || '', customerEmail: sale.customerEmail || '', paymentMethod: sale.paymentMethod || 'cash', discount: sale.discount || 0, items: (sale.items || []).map(i => ({ medicineId: i.medicineId, medicineName: i.medicineName, quantity: i.quantity, unitPrice: i.unitPrice })) });
  ...
  const invoiceSubtotal = invoiceForm.items.reduce((s, i) => s + i.unitPrice * i.quantity, 0);
  const invoiceTotal = invoiceSubtotal - Number(invoiceForm.discount || 0);

  const handleSaveInvoice = async () => {
    setSaving(true);
    try {
      const data = { ...invoiceForm, discount: Number(invoiceForm.discount), items: invoiceForm.items.map(i => ({ medicineId: i.medicineId, quantity: i.quantity })) };
      if (editSale) { await updateInvoice(editSale._id, data); success('Invoice updated'); } else { await createInvoice(data); success('Invoice created'); }
      setShowInvoiceEdit(false);
      fetchSales('invoice', filterStatus || null);
  ...
  const handlePayInvoice = async (id) => {
    ...
    try { await markInvoicePaid(id); success('Paid'); fetchSales('invoice', filterStatus || null); } catch (err) { error(err.response?.data?.message || 'Failed'); }
  };

  const handleCancelInvoice = async (id) => {
    if (!confirm('Cancel this invoice?')) return;
    try { await cancelSale(id); success('Cancelled'); fetchSales('invoice', filterStatus || null); } catch (err) { error('Failed'); }
  };

  const handleCancelSale = async (id) => {
    if (!confirm('Cancel this sale? Stock will be restored.')) return;
    try { await cancelSale(id); success('Cancelled'); fetchSales(null, 'paid,cancelled'); } catch (err) { error('Failed'); }
  };

  const handleDeleteInvoice = async (id) => {
    if (!confirm('Permanently delete this invoice? This cannot be undone.')) return;
    try { await cancelSale(id); success('Deleted'); fetchSales('invoice', filterStatus || null); } catch (err) { error('Failed'); }
  };

  const handleDeleteSaleRecord = async (id) => {
    if (!confirm('Permanently delete this record? This cannot be undone.')) return;
    try { await cancelSale(id); success('Deleted'); fetchSales(null, 'paid,cancelled'); } catch (err) { error('Failed'); }
  };

  const openEmailModal = (sale) => { setEmailTarget({ id: sale._id, email: sale.customerEmail || '' }); setShowEmailModal(true); };
  const handleSendInvoice = async () => {
    if (!emailTarget.email) { error('Email required'); return; }
    try { await sendInvoiceEmail(emailTarget.id, emailTarget.email); success('Sent'); setShowEmailModal(false); } catch (err) { error('Failed'); }
  };
  ...
  const printDoc = (title, content) => {
    ...
    const html = `<html><head><title>${title}</title><style>body{font-family:Arial,sans-serif;font-size:12px;padding:20px;max-width:360px;margin:0 auto;color:#1e293b;}.hdr{text-align:center;margin-bottom:12px;border-bottom:1px dashed #ccc;padding-bottom:8px;}.hdr h3{margin:0 0 4px;font-size:16px;}.hdr p{margin:0;font-size:11px;color:#64748b;}table{width:100%;border-collapse:collapse;}th{text-align:left;padding:4px 0;border-bottom:1px solid #ddd;font-size:10px;color:#64748b;}td{padding:3px 0;font-size:11px;border-bottom:1px solid #f1f5f9;}.r{text-align:right;}.b{font-weight:bold;}.tot{border-top:2px solid #1e293b;font-size:13px;}.ftr{text-align:center;margin-top:16px;font-size:9px;color:#94a3b8;border-top:1px dashed #ccc;padding-top:8px;}</style></head><body>${content}<div class="ftr">Generated by PharmaSys on ${now}</div></body></html>`;
...
<style>
  ...
  .hdr h3{margin:0 0 4px;font-size:16px;color:#059669;}
...
<div class="hdr"><h3>${systemName}</h3><p>Receipt #${sale.receiptNumber}</p><p>${formatDate(new Date())}</p></div>
...
<tbody>${(sale.items||[]).map(i=>`<tr><td>${i.medicineName||i.name}${i.dosage?' · '+i.dosage:''}</td><td class="r">${i.quantity}</td><td class="r">${formatCurrency(i.unitPrice||i.sellingPrice)}</td><td class="r b">${formatCurrency(i.total||i.totalPrice)}</td></tr>`).join('')}</tbody></table>
${sale.discount>0?`<p style="color:#e11d48;">Discount: -${formatCurrency(sale.discount)}</p>`:''}
<div class="tot" style="display:flex;justify-content:space-between;"><span>TOTAL</span><span style="color:#059669;">${formatCurrency(sale.totalAmount)}</span></div>
<p style="font-size:10px;color:#64748b;margin-top:4px;">Payment: ${sale.paymentMethod||'Cash'}</p>
<div class="ftr">Generated by PharmaSys at ${now}</div>
</body></html>`;
  const win = window.open('', '_blank', 'width=400,height=600');
  ...
  const printInvoice = (sale) => {
    ...
    printDoc(isPaid ? 'Receipt' : 'Invoice', `<div class="hdr"><h3>${systemName}</h3><p>${isPaid?'Receipt':'Invoice'} #${s.receiptNumber}</p><p>${formatDate(s.createdAt)}</p></div><div style="margin-bottom:12px;"><p style="margin:0;font-weight:bold;">${s.customerName||'Customer'}</p>${s.customerPhone?`<p style="margin:2px 0;font-size:11px;color:#64748b;">${s.customerPhone}</p>`:''}</div><table><thead><tr><th>Item</th><th class="r">Qty</th><th class="r">Price</th><th class="r">Total</th></tr></thead><tbody>${(s.items||[]).map(i => `<tr><td>${i.medicineName}${i.dosage?' · '+i.dosage:''}</td><td class="r">${i.quantity}</td><td class="r">${formatCurrency(i.unitPrice)}</td><td class="r b">${formatCurrency(i.totalPrice)}</td></tr>`).join('')}</tbody></table>${s.discount>0?`<div style="display:flex;justify-content:space-between;margin-top:8px;font-size:11px;"><span>Subtotal</span><span>${formatCurrency(s.subtotal)}</span></div><div style="display:flex;justify-content:space-between;color:#e11d48;font-size:11px;"><span>Discount</span><span>-${formatCurrency(s.discount)}</span></div>`:''}<div style="display:flex;justify-content:space-between;font-weight:bold;font-size:14px;margin-top:6px;border-top:1px solid #1e293b;padding-top:6px;"><span>${isPaid?'Total Paid':'Amount Due'}</span><span style="color:${isPaid?'#16a34a':'#f59e0b'};">${formatCurrency(s.totalAmount)}</span></div><div style="margin-top:4px;font-size:10px;color:#64748b;">Payment: ${s.paymentMethod||'Cash'} | Status: ${s.paymentStatus}</div>`);
  };
  ...
  const invoiceColumns = [
    ...
    { key: 'actions', label: '', render: (r) => (
      <div className="flex gap-1">
        <Button size="sm" variant="ghost" onClick={() => openInvoiceView(r)} title="View"><Eye size={14} /></Button>
        <Button size="sm" variant="ghost" onClick={() => printInvoice(r)} title="Print"><Printer size={14} /></Button>
        {r.paymentStatus === 'pending' && <Button size="sm" variant="ghost" onClick={() => openEditInvoice(r)} title="Edit"><Edit size={14} /></Button>}
        {r.paymentStatus === 'pending' && <Button size="sm" variant="ghost" onClick={() => handlePayInvoice(r._id)} className="text-green-500" title="Mark Paid"><CheckCircle size={14} /></Button>}
        {r.paymentStatus === 'pending' && <Button size="sm" variant="ghost" onClick={() => handleCancelInvoice(r._id)} className="text-red-500" title="Cancel"><Ban size={14} /></Button>}
        {r.paymentStatus === 'paid' && <Button size="sm" variant="ghost" onClick={() => handleDeleteInvoice(r._id)} className="text-red-500" title="Delete"><Trash2 size={14} /></Button>}
        <Button size="sm" variant="ghost" onClick={() => openEmailModal(r)} title="Email"><Send size={14} /></Button>
      </div>
    )},
  ];
  ...
  const historyColumns = [
    ...
    { key: 'actions', label: '', render: (r) => (
      <div className="flex gap-1">
        <Button size="sm" variant="ghost" onClick={() => openInvoiceView(r)} title="View"><Eye size={14} /></Button>
        <Button size="sm" variant="ghost" onClick={() => printInvoice(r)} title="Print"><Printer size={14} /></Button>
        {r.paymentStatus !== 'cancelled' && <Button size="sm" variant="ghost" onClick={() => handleCancelSale(r._id)} className="text-red-500" title="Cancel"><Ban size={14} /></Button>}
        {r.paymentStatus === 'paid' && <Button size="sm" variant="ghost" onClick={() => handleDeleteSaleRecord(r._id)} className="text-red-500" title="Delete"><Trash2 size={14} /></Button>}
      </div>
    )},
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'history' && (
        <div>
          {loading ? <Spinner /> : sales.length === 0 ? <Card className="text-center py-12"><FileText size={48} className="mx-auto text-gray-300 mb-4" /><h3 className="text-lg font-medium">No sales history</h3></Card> : <><Table columns={historyColumns} data={sales} /><Pagination page={page} totalPages={totalPages} onPageChange={setPage} /></>}
        </div>
      )}

      <Modal isOpen={showReceipt} onClose={() => setShowReceipt(false)} title="Receipt" size="sm">
        <div className="text-center border-b border-dashed pb-3 mb-3"><h3 className="font-bold text-base">{systemName}</h3><p className="text-xs text-gray-500">#{lastSale?.receiptNumber}</p><p className="text-xs text-gray-500">{formatDate(new Date())}</p></div>
        ...
        <div className="flex justify-between font-bold text-sm border-t pt-2 mt-2"><span>TOTAL</span><span className="text-primary-600">{formatCurrency(lastSale?.totalAmount)}</span></div>
      ...
      <Modal isOpen={showInvoiceEdit} onClose={() => setShowInvoiceEdit(false)} title={editSale?'Edit Invoice':'New Invoice'} size="lg">
        ...
        <div className="mb-4">
          <div className="relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><Input placeholder="Add medicine..." value={search} onChange={e=>setSearch(e.target.value)} className="pl-9"/></div>
          <div className="grid grid-cols-2 gap-1 mt-2 max-h-[150px] overflow-y-auto">{medicines.filter(m=>m.name.toLowerCase().includes(search.toLowerCase())).slice(0,10).map(med=><button key={med._id} onClick={()=>addInvoiceItem(med)} className="text-left p-2 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-sm">{med.name} - {formatCurrency(med.sellingPrice)}</button>)}</div>
        </div>
      ...
      <Modal isOpen={showInvoiceView} onClose={()=>setShowInvoiceView(false)} title={`Invoice #${viewSale?.receiptNumber}`} size="lg">
        <div className="text-center border-b pb-3 mb-4"><h3 className="font-bold text-lg">{systemName}</h3><p className="text-xs text-gray-500">Invoice #{viewSale?.receiptNumber}</p><p className="text-xs text-gray-500">{formatDate(viewSale?.createdAt)}</p></div>
        ...
        <table className="w-full text-sm"><thead><tr className="border-b-2"><th className="text-left py-2">Item</th><th className="text-center py-2">Qty</th><th className="text-right py-2">Price</th><th className="text-right py-2">Total</th></tr></thead><tbody>{viewSale?.items?.map((item,i)=><tr key={i} className="border-b"><td className="py-2">{item.medicineName}{item.dosage?` · ${item.dosage}`:''}</td><td className="text-center py-2">{item.quantity}</td><td className="text-right py-2">{formatCurrency(item.unitPrice)}</td><td className="text-right py-2 font-medium">{formatCurrency(item.totalPrice)}</td></tr>)}</tbody></table>
        <div className="mt-4 space-y-1 text-sm">{viewSale?.discount>0&&<><div className="flex justify-between"><span>Subtotal</span><span>{formatCurrency(viewSale.subtotal)}</span></div><div className="flex justify-between text-red-500"><span>Discount</span><span>-{formatCurrency(viewSale.discount)}</span></div></>}<div className="flex justify-between font-bold text-lg border-t pt-2"><span>{viewSale?.paymentStatus==='paid'?'Total Paid':'Amount Due'}</span><span className={viewSale?.paymentStatus==='paid'?'text-green-700':'text-yellow-600'}>{formatCurrency(viewSale?.totalAmount)}</span></div></div>
